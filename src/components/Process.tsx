...
export default function Process() {
  ...
  return (
    <section ref={sectionRef} id="process" className="relative bg-ground py-16 sm:py-20 lg:py-0 lg:min-h-[100dvh] flex flex-col justify-center overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-10 lg:mb-20">
        <motion.h2
          initial={{ y: 32, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true, margin: "-60px" }}
          transition={{ duration: 0.7, ease: [0.32, 0.72, 0, 1] }}
          className="font-display font-semibold text-[clamp(1.75rem,3.5vw,3rem)] tracking-[-0.02em] leading-[1.05] text-text-primary text-balance"
        >
      ...
      {isMobile ? (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 gap-3">
            {steps.map((step) => (
              <div key={step.number} className="double-bezel">
                <div className="double-bezel-inner p-4 h-full flex flex-col">
                  <span className="font-mono text-[10px] tracking-widest text-accent shrink-0">{step.number}</span>
                  <h3 className="font-display text-sm font-medium text-text-primary mt-2 mb-1.5">{step.title}</h3>
              ...
              <div className="double-bezel h-full">
                <div className="double-bezel-inner p-10 h-full flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="font-display text-[28px] font-medium text-text-primary mt-4 mb-3">{step.title}</h3>
                  </div>
                  <p className="text-[15px] text-text-secondary leading-relaxed max-w-[55ch]">{step.desc}</p>
                </div>
              </div>
            </div>
          ))}
